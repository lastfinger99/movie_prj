...
DROP SEQUENCE verification_idx_seq;
--인증요청 idx 시퀀스
CREATE SEQUENCE verification_idx_seq
INCREMENT BY 1
START WITH 1
MAXVALUE 999999999999999
CACHE 30
...
DROP SEQUENCE actor_code_idx_seq;
--배우 코드 idx 시퀀스
CREATE SEQUENCE actor_code_idx_seq
INCREMENT BY 1
START WITH 1
MAXVALUE 10000
CACHE 20
...
DROP SEQUENCE director_code_idx_seq;
--배우 코드 idx 시퀀스
CREATE SEQUENCE director_code_idx_seq
INCREMENT BY 1
START WITH 1
MAXVALUE 10000
CACHE 20
...
CREATE TABLE member(
    ...
    has_temp_pwd CHAR(1) DEFAULT 'N' NOT NULL,
...
CREATE TABLE non_member(
    ...
    ticket_pwd VARCHAR2(100) NOT NULL,
...
BEGIN --현재 날짜에 따라서 상태변경
    DBMS_SCHEDULER.CREATE_JOB(
        job_name => 'UPDATE_MOVIE_STATUS_DAILY',
        job_type => 'PLSQL_BLOCK',
        job_action => '
            BEGIN
                -- 상영예정 상태로 갱신
...
CREATE TABLE MOVIE (
    ...
    poster_path VARCHAR2(200) DEFAULT 'default_poster.png' NOT NULL,
...
CREATE TABLE movie_common_code_table (
    ...
    CONSTRAINT FK_MOVIE_TO_movie_common_code_table FOREIGN KEY (movie_idx) REFERENCES MOVIE (movie_idx) ON DELETE CASCADE,
    CONSTRAINT FK_MOVIE_COMMON_TABLE_TO_movie_common_code_table FOREIGN KEY (code_idx) REFERENCES movie_common_table (code_idx) ON DELETE CASCADE
);
...
--    CONSTRAINT FK_MOVIE_TO_MOIVE_PEOPLE_TABLE FOREIGN KEY (movie_idx) REFERENCES MOVIE (movie_idx) ON DELETE CASCADE,
--    CONSTRAINT FK_MOVIE_PEOPLE_COMMON_TABLE_TO_MOIVE_PEOPLE_TABLE FOREIGN KEY (people_code_idx) REFERENCES movie_people_common_table (people_code_idx) ON DELETE CASCADE
--);
...
CREATE TABLE schedule (
    ...
    remain_seats NUMBER DEFAULT 140 NOT NULL,
    CONSTRAINT PK_SCHEDULE PRIMARY KEY (schedule_idx),
    CONSTRAINT FK_MOVIE_TO_SCHEDULE FOREIGN KEY (movie_idx) REFERENCES MOVIE (movie_idx) ON DELETE CASCADE,
    CONSTRAINT FK_THEATER_TO_SCHEDULE FOREIGN KEY (theater_idx) REFERENCES theater (theater_idx ) ON DELETE CASCADE
);
...
CREATE TABLE saved_movie (
    user_idx NUMBER NOT NULL,
    movie_idx NUMBER NOT NULL,
    CONSTRAINT PK_SAVED_MOVIE PRIMARY KEY (user_idx, movie_idx),
...
CREATE TABLE reserved_seat (
    ...
    reservation_idx NUMBER NOT NULL,
    schedule_idx NUMBER NOT NULL,
    reserved_seat_status NUMBER NOT NULL,
...
--임시 좌석 테이블
Drop table temp_seat CASCADE CONSTRAINTS;
create table temp_seat (
	seat_idx NUMBER NOT NULL,
	schedule_idx NUMBER NOT NULL,
	click_time Timestamp DEFAULT SYSDATE NOT NULL,
	CONSTRAINT PK_temp_seat PRIMARY KEY (seat_idx,schedule_idx),
	CONSTRAINT FK_temp_seat FOREIGN KEY (schedule_idx) REFERENCES schedule (schedule_idx) ON DELETE CASCADE,
	CONSTRAINT FK_temp_seat2 FOREIGN KEY (seat_idx) REFERENCES seat (seat_idx) ON DELETE CASCADE
);
...
CREATE TABLE purchase_history (
    ...
    user_idx NUMBER NOT NULL,
    reservation_idx NUMBER NOT NULL,
    is_purchased NUMBER DEFAULT 1 NOT NULL,
...
'평범한 소년 팀은 성인이 되던 날, 아버지로부터 가문의 남자들은 시간을 되돌릴 수 있는 능력을 가지고 태어난다는 말을 듣게 된다.',  0, 'https://www.youtube.com/watch?v=7OIFdWk83no&pp=0gcJCdgAo7VqN5tD');

INSERT into MOVIE(MOVIE_IDX, MOVIE_NAME, RELEASE_DATE, COUNTRY, RUNNING_TIME, END_DATE, MOVIE_DESCRIPTION, SCREENING_STATUS, TRAILER_URL)
VALUES (movie_idx_seq.NEXTVAL, '레미제라블', '2025-05-30', '미국', 158, '2025-06-25', '빵 한 조각을 훔친 죄로 19년간의 감옥살이를 마친 후에도 전과자의 낙인이 찍힌 채 살아가는 장발장. 장발장을 끈질기게 추격하며 그의 새로운 삶을 뒤흔드는 자베르 경감. 주변 사람들의 모함으로 지독한 가난 속에서 아이를 위해 생존해가는 비운의 여인 판틴. 장발장에게 희망이 되어 준 판틴의 딸 코제트. 그리고 파리의 자유와 사랑을 위해 싸우는 마리우스, 에포닌, 앙졸라. 사랑과 용서, 구원과 희망을 꿈꾸는 그들의 노래가 스크린에 울려 퍼진다', 1, 'https://www.youtube.com/watch?v=y1LUXws8obU');

INSERT into MOVIE(MOVIE_IDX, MOVIE_NAME, RELEASE_DATE, COUNTRY, RUNNING_TIME, END_DATE, MOVIE_DESCRIPTION, SCREENING_STATUS, TRAILER_URL)
VALUES (movie_idx_seq.NEXTVAL, '승부', '2025-05-23', '대한민국', 114, '2025-06-05','천재 바둑기사와 그를 가르친 스승의 대결을 그린 휴먼 드라마', 0, 'https://www.youtube.com/watch?v=J8qqMLZPPTo');
...
VALUES (movie_idx_seq.NEXTVAL, '극한직업', '2025-05-15', '대한민국', 111, '2025-06-10','불철주야 달리고 구르지만 실적은 바닥, 급기야 해체 위기를 맞는 마약반! 더 이상 물러설 곳이 없는 팀의 맏형 고반장은 국제 범죄조직의 국내 마약 밀반입 정황을 포착하고 장형사, 마형사, 영호, 재훈까지 4명의 팀원들과 함께 잠복 수사에 나선다. 마약반은 24시간 감시를 위해 범죄조직의 아지트 앞 치킨집을 인수해 위장 창업을 하게 되고, 뜻밖의 절대미각을 지닌 마형사의 숨은 재능으로 치킨집은 일약 맛집으로 입소문이 나기 시작한다. 수사는 뒷전, 치킨장사로 눈코 뜰 새 없이 바빠진 마약반에게 어느 날 절호의 기회가 찾아오는데… 범인을 잡을 것인가, 닭을 잡을 것인가!',
 0, 'https://www.youtube.com/watch?v=xM1CIQd_X4c');
...
--슈퍼운영자
INSERT INTO admin(admin_id, user_idx, admin_pwd, admin_name, admin_email, manage_area, last_login_date, picture)
VALUES('superadmin', 1000000, 'superPwd', '난슈퍼운영자', 'superAdmin@gmail.com', '전체', SYSTIMESTAMP, null);


insert into MOVIE_PEOPLE_COMMON_TABLE (PEOPLE_CODE_IDX, PEOPLE_CODE_TYPE, PEOPLE_NAME) values (PEOPLE_CODE_IDX_seq.nextval, '배우', '장동권');
insert into MOVIE_PEOPLE_COMMON_TABLE (PEOPLE_CODE_IDX, PEOPLE_CODE_TYPE, PEOPLE_NAME) values (PEOPLE_CODE_IDX_seq.nextval, '배우', '김태희');
insert into MOVIE_PEOPLE_COMMON_TABLE (PEOPLE_CODE_IDX, PEOPLE_CODE_TYPE, PEOPLE_NAME) values (PEOPLE_CODE_IDX_seq.nextval, '배우', '원빈');


insert into MOVIE_PEOPLE_COMMON_TABLE (PEOPLE_CODE_IDX, PEOPLE_CODE_TYPE, PEOPLE_NAME) values (PEOPLE_CODE_IDX_seq.nextval, '감독', '봉준호');
insert into MOVIE_PEOPLE_COMMON_TABLE (PEOPLE_CODE_IDX, PEOPLE_CODE_TYPE, PEOPLE_NAME) values (PEOPLE_CODE_IDX_seq.nextval, '감독', '나준호');
insert into MOVIE_PEOPLE_COMMON_TABLE (PEOPLE_CODE_IDX, PEOPLE_CODE_TYPE, PEOPLE_NAME) values (PEOPLE_CODE_IDX_seq.nextval, '감독', '가준호');
